...
`,
  ...
  static args = {
    inputFolder: Args.string({
      description: "Input folder containing the SVG files",
      required: true,
    }),
    outputFolder: Args.string({
      description:
        "Output folder for the processed SVG files, will contain TSX files",
      required: true,
    }),
  };
  ...
  processSvgFiles(
    ...
    for (const fileName of files) {
      ...
      if (fs.statSync(inputFilePath).isFile()) {
        ...
        const modifiedContent = this.processSvgContent(
          baseName,
          content,
          changeParams
        );
  ...
  export default ${pascalCaseFilename};`;
    ...
    const newContent = content.replace(
      ...
      function (match: string, p1: string, p2: string, p3: string, p4: string) {
        const style = p3.replace(
          /-(\w)/g,
          function (match: string, p3: string) {
