...
describe("process", () => {
  ...
  before(() => {
    ...
    fs.writeFileSync(path.join(inputDir, "test.svg"), svgContent);
  });
  ...
  test
    ...
    .it("runs process cmd", (ctx) => {
      ...
      expect(outputFiles).to.include("test.tsx");
      ...
      expect(testTsxContent).to.equal(expectedContent);
    });
});
