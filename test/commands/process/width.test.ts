...
describe("process test width", () => {
  ...
  const expectedContent = `import React from "react";
    ...
    const TestWidth = ({ fill, width, height }: IIcons) => {
      return (
        <div><svg width="200" height="32" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  before(() => {
    ...
    fs.writeFileSync(path.join(inputDir, "test-width.svg"), svgContent);
  });
